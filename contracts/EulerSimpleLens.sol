...
// File contracts/modules/Exec.sol
...
contract Exec is BaseLogic {
    ...
    function liquidity(
        address account
    ) external staticDelegate returns (IRiskManager.LiquidityStatus memory status) {
        bytes memory result = callInternalModule(
            MODULEID__RISK_MANAGER,
            abi.encodeWithSelector(IRiskManager.computeLiquidity.selector, account)
        );
    ...
    function detailedLiquidity(
        address account
    ) public staticDelegate returns (IRiskManager.AssetLiquidity[] memory assets) {
        bytes memory result = callInternalModule(
            MODULEID__RISK_MANAGER,
    ...
    function getPrice(
        address underlying
    ) external staticDelegate returns (uint twap, uint twapPeriod) {
        bytes memory result = callInternalModule(
            MODULEID__RISK_MANAGER,
    ...
    function getPriceFull(
        address underlying
    ) external staticDelegate returns (uint twap, uint twapPeriod, uint currPrice) {
        bytes memory result = callInternalModule(
            MODULEID__RISK_MANAGER,
    ...
    function doBatchDispatch(
        ...
        for (uint i = 0; i < items.length; ++i) {
            ...
            bytes memory inputWrapped = abi.encodePacked(
                item.data,
                uint160(msgSender),
                uint160(proxyAddr)
            );
...
// File contracts/views/EulerSimpleLens.sol
...
contract EulerSimpleLens is Constants {
    bytes32 public immutable moduleGitCommit;
    Euler public immutable euler;
    Markets public immutable markets;
    Exec public immutable exec;
    ...
    function underlyingToInternalTokens(
        address underlying
    ) public view returns (address eToken, address dToken, address pToken) {
    ...
    function underlyingToAssetConfig(
        address underlying
    ) external view returns (Storage.AssetConfig memory config) {
    ...
    function interestRates(
        ...
        (borrowAPY, supplyAPY) = computeAPYs(
            borrowSPY,
            totalBorrows,
            totalBalances,
            reserveFee(underlying)
        );
    }
    ...
    function getAccountStatus(
        address account
    ) external view returns (uint collateralValue, uint liabilityValue, uint healthScore) {
    ...
    function getTotalSupplyAndDebts(
        ...
        public
        view
        returns (uint poolSize, uint totalBalances, uint totalBorrows, uint reserveBalance)
    {
    ...
    function computeAPYs(
        ...
        uint32 _reserveFee
    ) private pure returns (uint borrowAPY, uint supplyAPY) {
